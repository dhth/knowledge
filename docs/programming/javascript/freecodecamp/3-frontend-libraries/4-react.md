...
/*
There is a button element in the code editor which has an onClick() handler. This handler is triggered when the button receives a click event in the browser, and runs the handleClick method defined on MyComponent. Within the handleClick method, update the component state using this.setState().
*/
```
...
## Using previous state to make updates to it
...
class MyComponent extends React.Component {
  ...
  toggleVisibility = function(state, props){
    this.setState((state, props) => ({
...
**Note:** The `componentWillMount` Lifecycle method will be deprecated in a future version of 16.X and removed in version 17. [:fontawesome-solid-link: (Source)](https://reactjs.org/blog/2018/03/27/update-on-async-rendering.html)
...
[:fontawesome-solid-link: EventTarget.addEventListener()](https://developer.mozilla.org/en-US/docs/Web/API/EventTarget/addEventListener)

[:fontawesome-solid-link: Event reference](https://developer.mozilla.org/en-US/docs/Web/Events)
...
<video controls="controls" class="video"
       name="Video Name" src="https://github.com/dhth/knowledge-assets/blob/master/videos/freecodecamp/react-1.mov?raw=true"></video>

## Conditional Rendering

### Using `&&`

You can use the `&&` logical operator to perform conditional logic in a concise way. This is possible because you want to check if a condition is `true`, and if it is, return some markup. Here's an example:
...
<video controls="controls" class="video"
       name="Video Name" src="https://github.com/dhth/knowledge-assets/blob/master/videos/freecodecamp/react-2.mov?raw=true"></video>
